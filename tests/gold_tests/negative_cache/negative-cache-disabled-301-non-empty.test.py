...
request_header2 = {"headers": "GET /301 HTTP/1.1\r\nHost: www.example.com\r\n\r\n", "timestamp": "1469733493.993", "body": ""}
response_header2 = {"headers": "HTTP/1.1 301 Moved Permanently\r\nContent-Length: 3\r\nConnection: close\r\n\r\n", "timestamp": "1469733493.993", "body": "yyy"}
...
tr.Processes.Default.Streams.stdout = "gold/negative-cache-301-non-empty-cache-fill.gold"
...
tr.Processes.Default.Command = 'curl -s -D - -v --ipv4 --http1.1 -H "x-debug: x-cache,via" -H "Host: www.example.com" http://localhost:{port}/301'.format(port=ts.Variables.port)
tr.Processes.Default.ReturnCode = 0
tr.Processes.Default.Streams.stdout = "gold/negative-cache-301-non-empty-cache-hit.gold"
tr.StillRunningAfter = ts
...
tr.Processes.Default.Command = 'curl -s -D - -v --ipv4 --http1.1 -H "x-debug: x-cache,via" -H "Host: www.example.com" http://localhost:{port}/301'.format(port=ts.Variables.port)
tr.Processes.Default.ReturnCode = 0
tr.Processes.Default.Streams.stdout = "gold/negative-cache-301-non-empty-cache-ram-hit.gold"
tr.StillRunningAfter = ts
