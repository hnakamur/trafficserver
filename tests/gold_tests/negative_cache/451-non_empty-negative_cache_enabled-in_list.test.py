'''
Test negative cache 451 with negative_caching_enabled=1 and in list
'''
...
testName = ""
request_header1 = {"headers": "GET /451 HTTP/1.1\r\nHost: www.example.com\r\n\r\n", "timestamp": "1469733493.993", "body": ""}
response_header1 = {"headers": "HTTP/1.1 451 Unavailable For Legal Reasons\r\nContent-Length: 3\r\nConnection: close\r\n\r\n", "timestamp": "1469733493.993", "body": "yyy"}
...
ts.Disk.records_config.update({
    ...
    'proxy.config.http.negative_caching_list': '451',
...
# Test 1 - 451 non empty response and cache miss
tr = Test.AddTestRun()
tr.Processes.Default.StartBefore(server)
tr.Processes.Default.StartBefore(Test.Processes.ts, ready=1)
tr.Processes.Default.Command = 'curl -s -D - -v --ipv4 --http1.1 -H "x-debug: x-cache,via" -H "Host: www.example.com" http://localhost:{port}/451'.format(port=ts.Variables.port)
tr.Processes.Default.ReturnCode = 0
tr.Processes.Default.Streams.stdout = "gold/non_empty-cache_fill.gold"
tr.StillRunningAfter = ts

# Test 2 - 451 non empty response and cache miss
tr = Test.AddTestRun()
tr.Processes.Default.Command = 'curl -s -D - -v --ipv4 --http1.1 -H "x-debug: x-cache,via" -H "Host: www.example.com" http://localhost:{port}/451'.format(port=ts.Variables.port)
tr.Processes.Default.ReturnCode = 0
...
# Test 3 - 451 non empty response and cache miss
tr = Test.AddTestRun()
tr.Processes.Default.Command = 'curl -s -D - -v --ipv4 --http1.1 -H "x-debug: x-cache,via" -H "Host: www.example.com" http://localhost:{port}/451'.format(port=ts.Variables.port)
tr.Processes.Default.ReturnCode = 0
