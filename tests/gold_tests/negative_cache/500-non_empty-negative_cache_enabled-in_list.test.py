'''
Test negative cache 500 with negative_caching_enabled=1 and in list
'''
...
ts.Disk.records_config.update({
    ...
    'proxy.config.http.negative_caching_list': '500',
...
tr.Processes.Default.Streams.stdout = "gold/non_empty-cache_fill-no_date.gold"
...
tr.Processes.Default.Command = 'curl -s -D - -v --ipv4 --http1.1 -H "x-debug: x-cache,via" -H "Host: www.example.com" http://localhost:{port}/500'.format(port=ts.Variables.port)
tr.Processes.Default.ReturnCode = 0
tr.Processes.Default.Streams.stdout = "gold/non_empty-cache_hit-no_date-expires.gold"
tr.StillRunningAfter = ts
...
tr.Processes.Default.Command = 'curl -s -D - -v --ipv4 --http1.1 -H "x-debug: x-cache,via" -H "Host: www.example.com" http://localhost:{port}/500'.format(port=ts.Variables.port)
tr.Processes.Default.ReturnCode = 0
tr.Processes.Default.Streams.stdout = "gold/non_empty-cache_ram_hit-no_date-expires.gold"
tr.StillRunningAfter = ts
