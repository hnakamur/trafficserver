'''
Test negative cache 301 empty response with negative_caching_enabled=1, not in list and allow_empty_doc=1
'''
...
tr.Processes.Default.Streams.stdout = "gold/empty-cache_fill.gold"
...
tr.Processes.Default.Command = 'curl -s -D - -v --ipv4 --http1.1 -H "x-debug: x-cache,via" -H "Host: www.example.com" http://localhost:{port}/301'.format(port=ts.Variables.port)
tr.Processes.Default.ReturnCode = 0
tr.Processes.Default.Streams.stdout = "gold/empty-cache_hit.gold"
tr.StillRunningAfter = ts
...
tr.Processes.Default.Command = 'curl -s -D - -v --ipv4 --http1.1 -H "x-debug: x-cache,via" -H "Host: www.example.com" http://localhost:{port}/301'.format(port=ts.Variables.port)
tr.Processes.Default.ReturnCode = 0
tr.Processes.Default.Streams.stdout = "gold/empty-cache_ram_hit.gold"
tr.StillRunningAfter = ts
