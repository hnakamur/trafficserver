  ...
  if ((client_response_code == HTTP_STATUS_NOT_MODIFIED) || (client_response_code == HTTP_STATUS_PRECONDITION_FAILED)) {
    ...
    if (resp_via) {
      ...
      if (our_via == nullptr) {
        ...
        saved_our_via.append(our_via->value_get());
      ...
      if (saved_our_via.length()) {
        s->hdr_info.client_response.field_value_append(our_via, saved_our_via.c_str(), saved_our_via.length(), true);
      }
    }
