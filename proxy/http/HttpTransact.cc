...
find_server_and_update_current_info(HttpTransact::State *s)
{
  ...
  case PARENT_DIRECT:
    ...
    if (s->http_config_param->no_dns_forward_to_parent && !s->http_config_param->disable_forward_to_parent_when_empty) {
...
HttpTransact::CallOSDNSLookup(State *s)
{
  ...
  if (hst && hst->status == TSHostStatus::TS_HOST_STATUS_DOWN) {
    if (s->host_down_cache_fallback_attempted) {
      TxnDebug("http", "host down cache fallback already attempted; returning 502");
      build_error_response(s, HTTP_STATUS_BAD_GATEWAY, "Next Hop Connection Failed", "connect#failed_connect");
      s->next_action = SM_ACTION_SEND_ERROR_CACHE_NOOP;
      return;
    }
...
HttpTransact::will_this_request_self_loop(State *s, bool is_outbound_transparent)
{
  ...
  if (s->dns_info.lookup_success) {
    ...
    if (max_proxy_cycles == 0) {
      ...
      if (dst_port == local_port && (Machine::instance()->is_self(s->host_db_info.ip()) ||
                                     (!is_outbound_transparent && ats_ip_addr_eq(s->host_db_info.ip(), s->client_info.dst_addr)))) {
