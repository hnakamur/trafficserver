...
HttpSM::do_http_server_open(bool raw)
{
  ...
  // inside the loop check is skipped. The self-IP check (when max_proxy_cycles
  // == 0) and the Via-header multi-hop check are still enforced so a client
  // targeting ATS's own listening address or looping through its own Via UUID
  // is still caught.
  if (HttpTransact::will_this_request_self_loop(&t_state, ua_txn->is_outbound_transparent())) {
