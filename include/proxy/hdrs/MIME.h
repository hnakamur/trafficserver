...
MIMEField::name_set(HdrHeap *heap, MIMEHdrImpl *mh, std::string_view name)
{
  ...
  if (hdrtoken_is_wks(name.data())) {
    ...
    mime_field_name_set(heap, mh, this, name_wks_idx, name.data(), length, true);
  } else {
    int field_name_wks_idx = hdrtoken_tokenize(name.data(), length, &name_wks);
    mime_field_name_set(heap, mh, this, field_name_wks_idx, (field_name_wks_idx == -1 ? name.data() : name_wks), length, true);
  }
}
