...
public:
  DiagsLogContinuation() : Continuation(new_ProxyMutex())
  {
    ...
    auto configured_traffic_out_name{RecGetRecordStringAlloc("proxy.config.output.logfile.name").first};
...
adjust_sys_settings()
{
  ...
  if (maxfiles != RLIM_INFINITY) {
    auto [file_max_pct, err]{RecGetRecordFloat("proxy.config.system.file_max_pct")};
    if (err != REC_ERR_OKAY) {
      file_max_pct = 0.9;
    }
    if (file_max_pct > 1.0) {
...
adjust_num_of_net_threads(int nthreads)
{
  ...
  if (!nth_auto_config) {
    ...
    auto [autoconfig_scale, err]{RecGetRecordFloat("proxy.config.exec_thread.autoconfig.scale")};
    if (err != REC_ERR_OKAY) {
...
#define SET_INTERVAL(scope, name, var)                                    \
  ...
  auto p{RecGetRecordStringAlloc("proxy.config.diags.debug.client_ip").first};
  if (p) {
