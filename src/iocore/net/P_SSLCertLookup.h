...
struct SSLMultiCertConfigParams {
  SSLMultiCertConfigParams() : opt(SSLCertContextOption::OPT_NONE)
  {
    {
      RecInt val = 0;
      RecGetRecordInt("proxy.config.ssl.server.session_ticket.enable", &val);
      session_ticket_enabled = static_cast<int>(val);
    }
    {
      RecInt val = 0;
      RecGetRecordInt("proxy.config.ssl.server.session_ticket.number", &val);
      session_ticket_number = static_cast<int>(val);
    }
  }
