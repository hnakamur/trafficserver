...
LogConfig::read_configuration_variables()
{
  ...
  val = static_cast<int>(RecGetRecordInt("proxy.config.log.log_fast_buffer").first);
  if (val > 0) {
    log_fast_buffer = true;
  }

  val = static_cast<int>(RecGetRecordInt("proxy.config.log.max_secs_per_buffer").first);
  if (val > 0) {
    max_secs_per_buffer = val;
  }

  val = static_cast<int>(RecGetRecordInt("proxy.config.log.max_space_mb_for_logs").first);
  if (val > 0) {
    max_space_mb_for_logs = val;
  }

  val = static_cast<int>(RecGetRecordInt("proxy.config.log.max_space_mb_headroom").first);
  if (val > 0) {
    max_space_mb_headroom = val;
  }

  val = static_cast<int>(RecGetRecordInt("proxy.config.log.io.max_buffer_index").first);
  if (val > 0) {
...

  {
    ...
    int  logfile_perm_parsed = ink_fileperm_parse(str.c_str());
  ...
  {
    auto rec_str{RecGetRecordStringAlloc("proxy.config.error.logfile.filename").first};
    if (!rec_str.empty()) {
  ...
  val = static_cast<int>(RecGetRecordInt("proxy.config.log.preproc_threads").first);
  if (val > 0 && val <= 128) {
    preproc_threads = val;
  }
  ...
  //
  rolling_interval_sec = static_cast<int>(RecGetRecordInt("proxy.config.log.rolling_interval_sec").first);
  rolling_offset_hr    = static_cast<int>(RecGetRecordInt("proxy.config.log.rolling_offset_hr").first);
  rolling_size_mb      = static_cast<int>(RecGetRecordInt("proxy.config.log.rolling_size_mb").first);
  rolling_min_count    = static_cast<int>(RecGetRecordInt("proxy.config.log.rolling_min_count").first);
  val                  = static_cast<int>(RecGetRecordInt("proxy.config.log.rolling_enabled").first);
  ...
  val                      = static_cast<int>(RecGetRecordInt("proxy.config.log.auto_delete_rolled_files").first);
  auto_delete_rolled_files = (val > 0);

  val                 = static_cast<int>(RecGetRecordInt("proxy.config.log.rolling_allow_empty").first);
  rolling_allow_empty = (val > 0);

  // THROTTLING
  val = static_cast<int>(RecGetRecordInt("proxy.config.log.throttling_interval_msec").first);
  ...
  val = static_cast<int>(RecGetRecordInt("proxy.config.diags.debug.throttling_interval_msec").first);
  if (LogThrottlingIsValid(val)) {
  ...
  if (auto_delete_rolled_files) {
    ...
    // For diagnostic logs
    val = static_cast<int>(RecGetRecordInt("proxy.config.diags.logfile.rolling_min_count").first);
  ...
  val = static_cast<int>(RecGetRecordInt("proxy.config.log.file_stat_frequency").first);
  if (val > 0) {
    file_stat_frequency = val;
  }

  val = static_cast<int>(RecGetRecordInt("proxy.config.log.space_used_frequency").first);
  if (val > 0) {
    space_used_frequency = val;
  }

  // ASCII BUFFER
  val = static_cast<int>(RecGetRecordInt("proxy.config.log.ascii_buffer_size").first);
  if (val > 0) {
    ascii_buffer_size = val;
  }

  val = static_cast<int>(RecGetRecordInt("proxy.config.log.max_line_size").first);
  if (val > 0) {
    max_line_size = val;
  }
}
