...
{
  if (buf) {
    int64_t val = static_cast<int64_t>(RecGetRecordInt(config_var).first);
    marshal_int(buf, val);
  }
...
{
  auto str{RecGetRecordString_Xmalloc(config_var).first};
  auto len{LogAccess::strlen(str.data())};
  if (buf) {
    marshal_str(buf, str.data(), len);
  }
  ats_free(const_cast<char *>(str.data()));
  return len;
}
...
#define LOG_STRING  RECD_STRING
  ...
  if (RecGetRecordDataType(record, &stype) != REC_ERR_OKAY) {
    ...
    if (LOG_INTEGER == stype || LOG_COUNTER == stype) {
      ...
      if (auto [val, err]{RecGetRecordFloat(record)}; err == REC_ERR_OKAY) {
