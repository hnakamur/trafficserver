...
Log::periodic_tasks(long time_now)
{
  ...
  if (logging_mode_changed || Log::config->reconfiguration_needed) {
    ...
    if (logging_mode_changed) {
      int val = static_cast<int>(RecGetRecordInt("proxy.config.log.logging_enabled").first);
...
Log::init(int flags)
{
  ...
  if (config_flags & LOGCAT) {
    ...
    MgmtInt pti = RecGetRecordInt("proxy.config.log.periodic_tasks_interval").first;
    if (pti <= 0) {
