...
HttpSM::do_range_parse(MIMEField *range_field)
{
  ...
  calculate_output_cl(static_cast<int>(content_type.length()), num_chars_for_cl);
}
      ...
      if (do_transform) {
        if (api_hooks.get(TS_HTTP_RESPONSE_TRANSFORM_HOOK) == nullptr) {
          ...
          INKVConnInternal *range_trans = transformProcessor.range_transform(
            ...
            content_type.length(), content_length);
...
HttpSM::update_stats()
{
  ...
  */
  ...
  if (t_state.txn_conf->slow_log_threshold != 0 && ink_hrtime_from_msec(t_state.txn_conf->slow_log_threshold) < total_time) {
    ...
    auto field{t_state.hdr_info.client_request.value_get(
      std::string_view{MIME_FIELD_X_ID, static_cast<std::string_view::size_type>(MIME_LEN_X_ID)})};
    if (!field.empty()) {
      auto length = static_cast<int>(std::min(field.length(), sizeof(unique_id_string) - 1));
  ...
  if (is_redirect_required()) {
    if (redirect_url != nullptr || t_state.hdr_info.client_response.field_find(MIME_FIELD_LOCATION, MIME_LEN_LOCATION)) {
      ...
      if (redirect_url != nullptr) {
        ...
        redirect_request(redir_url.data(), redir_url.length());
      }
...
HttpSM::redirect_request(const char *arg_redirect_url, const int arg_redirect_len)
{
  ...
  if (t_state.hdr_info.server_request.valid()) {
    ...
    auto tmpOrigHost{t_state.hdr_info.server_request.value_get(
    ...
    if (tmpOrigHost.data()) {
      ...
      origHost[std::min(static_cast<int>(tmpOrigHost.length()), MAXDNAME - 1)] = '\0';
    } else {
