...
RemapProcessor::finish_remap(HttpTransact::State *s, UrlRewrite *table)
{
  ...
  if ((s->filter_mask & URL_REMAP_FILTER_REFERER) != 0 && (ri = map->referer_list) != nullptr) {
    std::string_view referer_hdr{nullptr, 0};
    ...
    if (request_header->presence(MIME_PRESENCE_REFERER) &&
        (referer_hdr = request_header->value_get(
           std::string_view{MIME_FIELD_REFERER, static_cast<std::string_view::size_type>(MIME_LEN_REFERER)}))
            .data() != nullptr) {
      if (referer_hdr.length() >= static_cast<int>(sizeof(tmp_referer_buf))) {
        referer_hdr = referer_hdr.substr(0, static_cast<int>(sizeof(tmp_referer_buf) - 1));
      }
        ...
        if ((s->filter_mask & URL_REMAP_FILTER_REDIRECT_FMT) != 0 && map->redir_chunk_list) {
          ...
          for (rc = map->redir_chunk_list; rc; rc = rc->next) {
            ...
            case 's':
              ...
              break;
            case 'r':
              c = (referer_hdr.length() && referer_hdr.data()) ? &tmp_referer_buf[0] : nullptr;
              break;
            case 'f':
            case 't':
  ...
  if (request_url && !host_hdr.empty() && s->txn_conf->maintain_pristine_host_hdr == 0) {
    if (dbg_ctl_url_rewrite.on()) {
      auto old_host_hdr{
        request_header->value_get(std::string_view{MIME_FIELD_HOST, static_cast<std::string_view::size_type>(MIME_LEN_HOST)})};
      if (old_host_hdr.length()) {
