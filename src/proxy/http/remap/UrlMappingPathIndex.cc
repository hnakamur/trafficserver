...
UrlMappingPathIndex::Insert(url_mapping *mapping)
{
  ...
  if (auto from_path{mapping->fromURL.path_get()};
      !trie->Insert(from_path.data(), mapping, mapping->getRank(), static_cast<int>(from_path.length()))) {
...
UrlMappingPathIndex::Search(URL *request_url, int request_port, bool normal_search /* = true */) const
{
  ...
  if (auto path{request_url->path_get()}; !(retval = trie->Search(path.data(), static_cast<int>(path.length())))) {
    Dbg(dbg_ctl_UrlMappingPathIndex_Search, "Couldn't find entry for url with path [%.*s]", static_cast<int>(path.length()),
        path.data());
    goto lFail;
  }
  return retval;
