...
TEST_CASE("HdrTest", "[proxy][hdrtest]")
{
  ...
  SECTION("Test mime")
  {
    ...
    std::string_view continuation = hdr.value_get(std::string_view{"continuation", 12});
    ...
    if (!continuation.substr(5).starts_with(std::string_view{"   ", 3})) {
      std::printf("FAILED: continue header unfolding did not produce correct WS's\n");
      REQUIRE(false);
    }

    if (!continuation.starts_with(std::string_view{"part1   part2", 13})) {
